...
export default function WorkflowBuilder() {
  ...
  const [logsFilter, setLogsFilter] = useState<
    "all" | "info" | "success" | "error" | "warning" | "system"
  >("all")
  ...
  useEffect(() => {
    const unsubscribeCreateWorkflow = wsClient.on(
    ...
    const unsubscribeExecutionLog = wsClient.on(
      "executionLog",
      (log: { message: string; type?: string; nodeId?: string; timestamp?: string }) => {
    ...
    const unsubscribeWorkflowComplete = wsClient.on("workflowComplete", (result: any) => {
      ...
      addExecutionLog(
        `Workflow execution completed: ${result.message || "Success"}`,
        "success",
        undefined,
        "workflow"
      )
    ...
    const unsubscribeWorkflowError = wsClient.on("workflowError", (error: any) => {
      setIsExecuting(false)
      addExecutionLog(
        `Workflow error: ${error.message || "Unknown error"}`,
        "error",
        undefined,
        "workflow"
      )
  ...
  const createWorkflowFromDSL = useCallback(
    (result: DSLAnalysisResult) => {
      ...
      if (result.matchedAgents && result.matchedAgents.length > 0) {
        result.matchedAgents.forEach((match: any, index: number) => {
          ...
          p2pAgentNode.data = {
            ...
            description:
              match.reasoning ||
              `P2P Agent with capabilities: ${match.matchedTools?.join(", ") || "various"}`,
            status: "idle",
            agentName: agentName,
        ...
        // fallback P2P agent
        let defaultAgentName = "go-agent-2"
        if ((result as any).availableAgents && (result as any).availableAgents.length > 0) {
          const availableP2PAgent = (result as any).availableAgents.find(
            (agent: any) =>
              agent.agentName !== "orchestrator" && agent.agent_name !== "orchestrator"
          )
          if (availableP2PAgent) {
            defaultAgentName =
              availableP2PAgent.agentName || availableP2PAgent.agent_name || defaultAgentName
          }
        }
        ...
        defaultP2PAgent.data = {
          ...defaultP2PAgent.data,
          label: "P2P Executor",
          description: `P2P Agent with MCP tools: ${
            result.requiredMCPTools?.map((t) => t.name).join(", ") || "filesystem"
          }`,
          status: "idle",
      ...
      if (result.requiredMCPTools && result.requiredMCPTools.length > 0) {
        ...
        toolNode.data = {
          ...
          toolId: result.requiredMCPTools[0].name,
          parameters: {
  ...
  const generateWorkflowCreationMessage = (
    nodes: WorkflowNode[],
    edges: WorkflowEdge[],
    result: DSLAnalysisResult
  ): string => {
  ...
  const getExecutionOrder = () => {
    ...
    const adjList = new Map(nodes.map((node) => [node.id, [] as string[]]))
  ...
  const executeWorkflow = async () => {
    ...
    edges.forEach((edge, index) => {
      ...
      addExecutionLog(
        `📌 Connection ${index + 1}: ${sourceNode?.data.label || edge.source} → ${
    ...
    try {
      // pull params/secrets from localStorage (managed by chat panel)
      const params = safeParseJSON<Record<string, any>>(localStorage.getItem("praxis.params"), {})
      const secrets = safeParseJSON<Record<string, string>>(
        localStorage.getItem("praxis.secrets"),
        {}
      )

      // ✅ Log ONLY params (never secrets)
      if (params && Object.keys(params).length > 0) {
        addExecutionLog(`🧩 Params provided: ${JSON.stringify(params)}`, "info", undefined, "workflow")
      } else {
        addExecutionLog("🧩 No params provided.", "info", undefined, "workflow")
      }
      ...
      if (sent) {
        addExecutionLog(
          `📡 Workflow execution request sent to backend (with params & secrets if provided)`
        )
  ...
  const updateNodeStatus = (
    ...
    setNodes((nodes) =>
      nodes.map((node) => (node.id === nodeId ? { ...node, data: { ...node.data, status } } : node))
    )
  }
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      setNodes((nds) => nds.concat(newNode as WorkflowNode))
    },
    [setNodes]
  )
          ...
          >
            ...
            <Panel position="top-left">
              <div className="text-xs text-gray-600 bg-white/80 backdrop-blur-sm p-2 rounded-md border border-gray-200">
                ...
                {isExecuting && (
                  <span className="ml-2 text-blue-600 font-medium">⚡ Executing...</span>
                )}
              </div>
            </Panel>
              ...
              <div className="max-h-48 overflow-y-auto space-y-1 text-xs bg-gray-900/50 p-3 rounded border border-gray-700">
                {getFilteredLogs().map((log) => {
                  ...
                  return (
                    <div
                      key={log.id}
                      className={`opacity-90 hover:opacity-100 border-l-2 pl-3 py-1 transition-all duration-200 ${getLogStyles(
                        log.type
                      )}`}
