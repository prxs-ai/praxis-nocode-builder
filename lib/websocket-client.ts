...
export class WebSocketClient {
  ...
  private context: { params: ParamBag; secrets: SecretBag } = { params: {}, secrets: {} }

  constructor(url: string = process.env.NEXT_PUBLIC_WS_URL || "ws://localhost:8090/ws/workflow") {
    this.url = url
    this.initializeEventMaps()
    setTimeout(() => {
      this.connect().catch((error) => {
        console.warn("Auto-connect failed:", (error as Error)?.message)
      })
    }, 100)
  }

  public setContext(ctx: WithParamsAndSecrets): void {
    this.context.params = { ...(this.context.params || {}), ...(ctx.params || {}) }
    this.context.secrets = { ...(this.context.secrets || {}), ...(ctx.secrets || {}) }
  }

  public clearContext(): void {
    this.context = { params: {}, secrets: {} }
  }

  private mergeParamsAndSecrets<T extends object>(payload: T, extra?: WithParamsAndSecrets): T & WithParamsAndSecrets {
    const mergedParams = { ...(this.context.params || {}), ...(extra?.params || {}) }
    const mergedSecrets = { ...(this.context.secrets || {}), ...(extra?.secrets || {}) }
    const out: any = { ...payload }
    if (Object.keys(mergedParams).length) out.params = mergedParams
    if (Object.keys(mergedSecrets).length) out.secrets = mergedSecrets
    return out
  }
  ...
  /**
   * Chat + optional per-call params/secrets
   */
  public sendChatMessage(content: string, ctx?: WithParamsAndSecrets): boolean {
    const basePayload = {
      content,
      sender: "user",
      timestamp: new Date().toISOString(),
    }
    const payload = this.mergeParamsAndSecrets(basePayload, ctx)
  ...
  /**
   * DSL command + optional per-call params/secrets
   */
  public sendDSLCommand(payload: DSLCommandPayload, ctx?: WithParamsAndSecrets): boolean {
    const merged = this.mergeParamsAndSecrets(payload, ctx)
    const message: WebSocketMessage = {
      type: "DSL_COMMAND",
      payload: merged,
      timestamp: new Date().toISOString(),
  ...
  /**
   * Execute workflow (nodes/edges OR stored workflow via ctx.workflow),
   * with optional params/secrets overrides
   */
  public startWorkflowExecution(
    workflowId: string,
    nodes: any[],
    edges?: any[],
    ctx?: WithParamsAndSecrets & { workflow?: { id: string } }
  ): boolean {
    const base = ctx?.workflow
      ? { workflow: ctx.workflow, startTime: Date.now() }
      : { workflowId, nodes, edges: edges || [], startTime: Date.now() }

    const payload = this.mergeParamsAndSecrets(base, ctx)
    const message: WebSocketMessage = {
  ...
  private processMessage(message: WebSocketMessage): void {
    try {
      switch (message.type) {
        ...
        case "WORKFLOW_LOG":
          if (message.payload) {
            const logMessage = `[${message.payload.level?.toUpperCase() || "INFO"}] ${
              message.payload.source || "System"
          ...
          if (message.payload) {
            ...
            const completionMessage =
              typeof message.payload.message === "string"
                ...
                ? typeof message.payload.results === "string"
                  ? message.payload.results
                  : "Workflow completed"
        ...
        case "WORKFLOW_ERROR":
          if (message.payload) {
            ...
            this.emit("addAssistantMessage", {
              content: `❌ Ошибка выполнения воркфлоу: ${
                message.payload.message || message.payload.error || "Неизвестная ошибка"
              }`,
              type: "error",
              timestamp: new Date().toISOString(),
            })
      ...
      const errorData = {
        ...
        code: "PARSE_ERROR",
        originalError: error,
        // @ts-expect-error event may not be in this scope when thrown elsewhere; safe to omit
        rawData: (event as any)?.data,
      }
      this.emit("error", errorData)
    }
  }
...
export const createWebSocketClient = (url?: string): WebSocketClient => {
  return new WebSocketClient(url || process.env.NEXT_PUBLIC_WS_URL || "ws://localhost:8090/ws/workflow")
}